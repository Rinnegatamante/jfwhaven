		...
		switch (ev.type) {
			...
			*/
			case SDL_CONTROLLERAXISMOTION:
				//if (appactive) {
				if (ev.caxis.axis == 0) { // Move Left/Right
					if (ev.caxis.value < -16000) {
						SetKey(0x33, 1);
						SetKey(0x34, 0);
					} else if (ev.caxis.value > 16000) {
						SetKey(0x33, 0);
						SetKey(0x34, 1);
					} else {
						SetKey(0x33, 0);
				...
				} else if (ev.caxis.axis == 1) { // Move Forward/Back
					if (ev.caxis.value < -16000) {
						SetKey(0xC8, 1);
						SetKey(0xD0, 0);
					} else if (ev.caxis.value > 16000) {
				...
				} else if (ev.caxis.axis == 2) { // Move Left/Right Camera
					if (ev.caxis.value < -16000) {
						SetKey(0xCB, 1);
						SetKey(0xCD, 0);
					} else if (ev.caxis.value > 16000) {
				...
				} else if (ev.caxis.axis == 3) { // Move Up/Down Camera
					if (ev.caxis.value < -16000) {
						SetKey(0xC9, 1);
						SetKey(0xD1, 0);
					} else if (ev.caxis.value > 16000) {
			...
			case SDL_CONTROLLERBUTTONDOWN:
				//if (appactive) {
					...
					case SDL_CONTROLLER_BUTTON_DPAD_UP: // Use Potion
						SetKey(0x13, 1);
						break;
					...
					case SDL_CONTROLLER_BUTTON_A: // Jump
						SetKey(0x1C, 1);
						break;
					case SDL_CONTROLLER_BUTTON_B: // Crouch
						SetKey(0x2E, 1);
						break;
					case SDL_CONTROLLER_BUTTON_X: // Use
						SetKey(0x39, 1);
						break;
					case SDL_CONTROLLER_BUTTON_Y: // Fly
						SetKey(0x23, 1);
						break;
					case SDL_CONTROLLER_BUTTON_BACK: // Map
						SetKey(0x0F, 1);
						break;
					case SDL_CONTROLLER_BUTTON_START: // ESC
						SetKey(0x01, 1);
						break;
					case SDL_CONTROLLER_BUTTON_LEFTSHOULDER: // Cast
						SetKey(0x16, 1);
						break;
					case SDL_CONTROLLER_BUTTON_RIGHTSHOULDER: // Fire
						SetKey(0x1D, 1);
						break;
					default:
						break;
